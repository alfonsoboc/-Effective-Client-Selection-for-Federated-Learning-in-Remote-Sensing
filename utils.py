...
from sampling import eurosatiid, eurosatnoniid, eurosatpar
...
def get_dataset(args,seed=None):
    ...
    if args.dataset == 'cifar':
        args.num_classes = 10
        data_dir = '/home/daniel/testdir/FedCor/src/data/cifar10'
        ...
        data_dir = '/home/daniel/testdir/FedCor/src/data/'
 ...
 #IMPORTANT PART OF THE CODE: HERE ARE THE DIFFERENT OPTIONS FOR THE EUROSAT USE OF DATASET
 #CHANGE THE data_dir VALUE FOR YOUR CASE!
    elif args.dataset == 'eurosat':
        data_dir = 'C:\\Users\\a\\OneDrive\\Escritorio\\PCVRS_submission\\src\\data' 
        args.num_classes = 10
        
        # Sample training data amongst users
        if args.iid==1:
            train_dataset, test_dataset, user_groups = eurosatiid(data_dir, args.num_users, rs)
            _, _, user_groups_test = eurosatiid(data_dir, args.num_users, rs)
        else:
            if args.alpha is not None:
                raise NotImplementedError("Dirichlet non-IID sampling not implemented for EuroSAT")
            elif args.unequal:
                raise NotImplementedError()
            elif args.shards_per_client:
                train_dataset, test_dataset, user_groups = eurosatpar(data_dir, args.num_users, args.shards_per_client, rs)
                _, _, user_groups_test = eurosatpar(data_dir, args.num_users, args.shards_per_client, rs)
            else:
                train_dataset, test_dataset, user_groups = eurosatnoniid(data_dir, args.num_users)
                _, _, user_groups_test = eurosatnoniid(data_dir, args.num_users)
